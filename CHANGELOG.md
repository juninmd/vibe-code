...
### Fixed
- **Session board na UI real** (achados da homologação no browser): header quebrava em telas estreitas e empurrava o botão `Close` para fora da tela (agora quebra em duas linhas e a régua de filtros rola na horizontal); overlay translúcido deixava o board por baixo aparecer entre as colunas (agora usa o fundo do app); títulos longos esticavam o card (limitados a 3 linhas); contador `0 msg` aparecia em sessão sem mensagens; falha de scan mostrava "nenhuma sessão encontrada" como se a máquina estivesse vazia (agora tem estado próprio com `Try again`); Antigravity caía no ícone genérico (ganhou ícone e cor próprios); tempos relativos ganharam o timestamp absoluto no tooltip.
...
### Added
- **Session Board homologado ponta a ponta**: `e2e/sessions.spec.ts` cobre API + UI (colunas, filtro por CLI e por texto, copiar comando de resume, fechar/reabrir com `S`, estado de falha) sobre stores determinísticos das três CLIs semeados pelo `playwright.config.ts`. Para imagens de CI com browser pré-instalado, `VIBE_E2E_CHROMIUM` sobrescreve o executável do Chromium.
