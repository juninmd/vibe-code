...
export class KimiEngine implements AgentEngine {
  name = "kimi";
  displayName = "Kimi";
  ...
  async *execute(
    ...
    yield { type: "log", stream: "system", content: `[kimi] Starting in ${workdir}` };

    const promptFile = `/tmp/vibe-kimi-prompt-${options.runId ?? Date.now()}.txt`;
