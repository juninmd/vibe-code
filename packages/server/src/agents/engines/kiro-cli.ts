...
export class KiroCliEngine implements AgentEngine {
  name = "kiro-cli";
  ...
  async isAvailable(): Promise<boolean> {
    try {
      const proc = Bun.spawn(["kiro-cli", "--version"], { stdout: "pipe", stderr: "pipe" });
      await proc.exited;
  ...
  async *execute(
    ...
    yield { type: "log", stream: "system", content: `[kiro-cli] Starting in ${workdir}` };

    const promptFile = `/tmp/vibe-kiro-cli-prompt-${options.runId ?? Date.now()}.txt`;
    await writeFile(promptFile, prompt, "utf8");

    const args = ["kiro-cli", "acp"];
