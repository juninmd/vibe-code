...
export class BaseCliEngine implements AgentEngine {
  constructor(
    public name: string,
  ...
  async isAvailable(): Promise<boolean> {
    try {
      const proc = Bun.spawn([this.cliCommand, "--version"], { stdout: "pipe", stderr: "pipe" });
      await proc.exited;
  ...
  async *execute(
    ...
    yield { type: "log", stream: "system", content: `[${this.name}] Starting in ${workdir}` };

    const args = [this.cliCommand];
