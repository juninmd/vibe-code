...
export function createWorkspaceQueries(db: Database) {
  return {
    list: (): Workspace[] => {
      const rows = db.prepare<WorkspaceRow, []>("SELECT * FROM workspaces ORDER BY created_at DESC").all();
      return rows.map(mapWorkspace);
    },
    get: (id: string): Workspace | null => {
      const row = db.prepare<WorkspaceRow, [string]>("SELECT * FROM workspaces WHERE id = ?").get(id);
      return row ? mapWorkspace(row) : null;
    },
    getBySlug: (slug: string): Workspace | null => {
      const row = db.prepare<WorkspaceRow, [string]>("SELECT * FROM workspaces WHERE slug = ?").get(slug);
      return row ? mapWorkspace(row) : null;
    },
