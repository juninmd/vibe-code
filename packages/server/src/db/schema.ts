...
Ao terminar, rode "npm run build" para garantir zero erros de compilação.', 'app', 1),
    ...
    ('bug-fix-tdd', 'Corrigir Bug (TDD)', 'Reproduz, corrige e prova o bug com teste', 'Corrija o bug descrito abaixo seguindo TDD:\n\n1. Reproduza o problema escrevendo primeiro um teste que falha e demonstra o bug.\n2. Implemente a menor correção possível para o teste passar.\n3. Rode toda a suíte para garantir que nada quebrou.\n4. Explique a causa raiz em uma frase no corpo do commit.\n\nBug:\n[descreva o comportamento atual vs esperado, passos para reproduzir e logs]', 'code', 1),
    ('dependency-upgrade', 'Atualizar Dependências', 'Atualiza pacotes com segurança e valida build', 'Atualize as dependências deste projeto com segurança:\n\n- Identifique pacotes desatualizados e vulnerabilidades conhecidas.\n- Atualize primeiro patches/minors, depois majors um a um.\n- Para cada major, leia o changelog e ajuste o código às breaking changes.\n- Rode lint, typecheck, testes e build após cada grupo de mudanças.\n- Resuma o que mudou e qualquer risco residual.', 'devops', 1),
    ('conventional-pr', 'Preparar Pull Request', 'Gera título e descrição de PR a partir do diff', 'Analise o diff atual e prepare um Pull Request de alta qualidade:\n\n- Título seguindo Conventional Commits (feat/fix/chore/refactor...).\n- Descrição com: contexto/motivação, o que mudou, como testar e riscos.\n- Lista de checagens feitas (lint, typecheck, testes, build).\n- Destaque mudanças que exigem atenção do revisor.\n\nMantenha objetivo e em português.', 'code', 1),
    ('e2e-tests', 'Testes End-to-End', 'Cria testes E2E dos fluxos críticos', 'Escreva testes end-to-end para os fluxos críticos desta aplicação usando o framework já presente no projeto (Playwright/Cypress/etc.):\n\n- Cubra os caminhos felizes principais e ao menos um caminho de erro por fluxo.\n- Use seletores acessíveis e estáveis (roles/labels), evite seletores frágeis.\n- Isole o estado entre os testes (setup/teardown).\n- Garanta que rodam em CI de forma determinística.', 'code', 1),
    ('dockerfile-optimize', 'Otimizar Dockerfile', 'Reduz imagem e melhora segurança do container', 'Otimize o Dockerfile e a configuração de container deste projeto:\n\n- Use multi-stage build e imagens base mínimas.\n- Ordene camadas para maximizar cache e minimizar rebuilds.\n- Rode como usuário não-root e remova ferramentas de build do runtime.\n- Garanta .dockerignore adequado e ausência de segredos na imagem.\n- Valide com hadolint/trivy se disponível e documente o tamanho final.', 'devops', 1)
  `);
