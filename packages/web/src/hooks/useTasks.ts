...
import type { TaskWithRun, CreateTaskRequest, UpdateTaskRequest } from "@vibe-code/shared";
...
export function useTasks(repoFilter?: string) {
  ...
  const launchTask = useCallback(async (id: string, engine?: string) => {
    const run = await api.tasks.launch(id, engine ? { engine } : undefined);
    await refresh();
    return run;
  }, [refresh]);
